...
#if defined (__CUDA_ARCH__) && (__CUDA_ARCH__ < 500) //when double atomicAdd was added as part of the main CUDA library.
  ...
  _functionReduceAtom(T *sum, T *g_idata, const unsigned int n)
{
  ...
  //printf("\nFRA Statistics \nthreadIdx.x: %d \nblockIdx.x: %d \nblockDim.x %d", threadIdx.x, blockIdx.x, blockDim.x);
  ...
  if (threadIdx.x == 0){ 
	  __syncthreads();
	  //printf("Last Resort Global Memory Transfer\n");
