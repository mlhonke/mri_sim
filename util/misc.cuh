...
__device__ __host__ inline int sgn(real val){
...
inline void safe_cuda(cudaError_t a, std::string comment){
	if (a != cudaSuccess) {
...
inline void printDeviceInfo(int device_num){
...
inline double linear_regression(std::vector<real> y, std::vector<real> x){

	if ( y.size() != x.size() ) { std::cout << "ERROR LINEAR_REGRESSION Y.SIZE != X.SIZE " << std::endl; exit(0); }
	double term1 = (double) y.size(), term2 = 0.0, term3 = 0.0, term4 = 0.0, term5 = 0.0, delta;
...
__host__ inline void cudaLastErrorCheck(){
...
inline double unifRandCPP(double a, double b)
{
