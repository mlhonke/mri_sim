#ifndef _INF_CYLINDER_XY_H_
#define _INF_CYLINDER_XY_H_

#include "util/misc.cuh"
#include "util/compare.cuh"

class infCylinder_XY {
...
public:
  ...
  __device__ __host__ infCylinder_XY(){}
  __device__ __host__ infCylinder_XY(real _centerX, real _centerY, real _radius, real _T2, real _T1, real _D, int _region, real _permeability){
    centerX = _centerX;
    centerY = _centerY;
    radius = _radius;
    D = _D;
  ...
  __device__ __host__ infCylinder_XY(real _centerX, real _centerY, real _radius, real _T2, real _T1, real _D, int _region, real _permeability, real _eps){
    centerX = _centerX;
    centerY = _centerY;
    radius = _radius;
    D = _D;
  ...
  __device__ __host__ bool inside(const Vector3 & r) const{
    ...
    return ( (r.x - centerX)*(r.x - centerX) + (r.y - centerY)*(r.y - centerY) < radius*radius );
  ...
  __device__ __host__ bool intersect(const Vector3 & ri, const Vector3 & rf, real & v, Vector3 & n) const{
    ...
    real a = dr.x*dr.x + dr.y*dr.y;
    real b = 2.0*ri.x*dr.x - 2.0*dr.x*centerX + 2.0*ri.y*dr.y - 2.0*dr.y*centerY;
    real c = ri.x*ri.x + ri.y*ri.y -2.0*ri.x*centerX -2.0*ri.y*centerY + centerX*centerX + centerY*centerY - radius*radius; 
  ...
  __device__ void randUnif(Vector3 & r, curandState & localState) const{
	do {
	r = Vector3((2.0*curand_uniform(&localState)-1.0)*radius,(2.0*curand_uniform(&localState)-1.0)*radius,(2.0*curand_uniform(&localState)-1.0)*radius) + getCenter();
	} while(!inside(r));
  }
  ...
  __host__ void randUnif(Vector3 & r) const{
	...
	do {
	r = Vector3((2.0*unifRandCPP()-1.0)*radius,(2.0*unifRandCPP()-1.0)*radius,z) + getCenter();
	} while(!inside(r));
  }
