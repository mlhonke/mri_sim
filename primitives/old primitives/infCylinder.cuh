...
public:
  ...
  __device__ __host__ infCylinder(){}
  __device__ __host__ infCylinder(Vector3 _center_simul, Vector3 _axis, real _radius, real _T2, real _T1, real _D, int _region, real _permeability){
		...
		if( doub_equal(ahat.x,0.0) && doub_equal(ahat.y,0.0) && doub_equal(ahat.z,0.0) ){
		...
		center_cyl = getCylinderCoordinates(_center_simul); //center of cylinder in cylinder coordinates (ahat,bhat,chat basis)
  ...
  __device__ Vector3 unifRand(curandState localState) const{
	...
	do {
	r_cyl.x = radius*(2.0*curand_uniform(&localState) - 1) + center_cyl.x;
	r_cyl.y = radius*(2.0*curand_uniform(&localState) - 1) + center_cyl.y;
  ...
  __device__ __host__ bool intersect(const Vector3 ri_simul, const Vector3 rf_simul, real *v) const{
    ...
    real a = dr.x*dr.x + dr.y*dr.y;
    real b = 2.0*ri.x*dr.x - 2.0*dr.x*center_cyl.x + 2.0*ri.y*dr.y - 2.0*dr.y*center_cyl.y;
    real c = ri.x*ri.x + ri.y*ri.y -2.0*ri.x*center_cyl.x -2.0*ri.y*center_cyl.y + center_cyl.x*center_cyl.x + center_cyl.y*center_cyl.y - radius*radius; 
