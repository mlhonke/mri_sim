...
public:
  ...
  __device__ __host__ Sphere(real _centerX, real _centerY, real _centerZ, real _radius, real _T2, real _T1, real _D, int _region, real _permeability){
  ...
  __device__ __host__ Sphere(real _centerX, real _centerY, real _centerZ, real _radius, real _T2, real _T1, real _D, int _region, real _permeability, real _eps){
  ...
  __host__ Vector3 unifRandCPU() const{
	...
	while ( !inside(r) ){
		...
		r = Vector3( radius*(2.0*unifRandCPP()  - 1), radius*(2.0*unifRandCPP()  - 1), radius*(2.0*unifRandCPP() - 1));
  ...
  __device__ __host__ bool inside(const Vector3 & r) const{
    return ((r.x - centerX)*(r.x - centerX) + (r.y - centerY)*(r.y - centerY) + (r.z - centerZ)*(r.z - centerZ) < radius*radius);
  }
  
   __device__ __host__ bool inside(real x, real y, real z) const{
    return ((x - centerX)*(x - centerX) + (y - centerY)*(y - centerY) + (z - centerZ)*(z - centerZ)< radius*radius);
  }
  ...
  __device__ __host__ bool intersect(const Vector3 & ri, const Vector3 & rf, real & v) const{
	...
	real b = 2.0*ri.x*dr.x - 2.0*dr.x*centerX + 2.0*ri.y*dr.y - 2.0*dr.y*centerY + 2.0*ri.z*dr.z - 2.0*dr.z*centerZ;
	real c = ri.x*ri.x + ri.y*ri.y +ri.z*ri.z -2*ri.x*centerX -2*ri.y*centerY -2*ri.z*centerZ  + centerX*centerX + centerY*centerY + centerZ*centerZ - radius*radius;
		
    real q = -.5*(b + sgn(b)*sqrt(b*b - 4*a*c));
    real root1 = q/a;
    real root2 = c/q;

    bool s1 = (root1 > 0.0 && root1 < 1.0 && b*b>4*a*c && !real_equal(root1*step_mag,0.0,sphereEPS));
    bool s2 = (root2 > 0.0 && root2 < 1.0 && b*b>4*a*c && !real_equal(root2*step_mag,0.0,sphereEPS));
  ...
  __host__ void setEPS(real _sphereEPS){
	sphereEPS = _sphereEPS;
  }
  ...
  __device__ void randUnif(Vector3 & r, curandState & localState) const{
	...
	r = Vector3((2.0*curand_uniform(&localState)-1.0)*radius,
			...
			(2.0*curand_uniform(&localState)-1.0)*radius) + getCenter();
	} while(!inside(r));
  }
  ...
  __host__ void randUnif(Vector3 & r) const{
	...
	r = Vector3((2.0*unifRandCPP()-1.0)*radius,
			  ...
			  (2.0*unifRandCPP()-1.0)*radius) + getCenter();
	} while(!inside(r));
  }
